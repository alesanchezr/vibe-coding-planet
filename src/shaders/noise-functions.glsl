        ...
        float fractal3(
          ...
          // Normalize noise between [-1.0, 1.0] approx.
           // Avoid division by zero if max_amp is somehow zero
          return max_amp > 0.0 ? n / max_amp : 0.0;
        }

        // Note: terrainHeight function is not needed for atmosphere shaders
        // float terrainHeight(...) { ... }